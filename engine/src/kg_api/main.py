...
def get_note(note_id: str) -> NoteDetail:
    ...
    conns = [_conn_out(c) for c in _eng().surfaced() if c.a_id == note_id or c.b_id == note_id]
...
def find_connections(note_id: str) -> JobOut:
    ...
    surfaced = [c for c in _eng().surfaced() if c.a_id == note_id or c.b_id == note_id]
