...
| Embeddings | **Eval-gated, versioned embedder seam** (bake-off winner; 768-d local opt-in) | *Revised — see `BACKEND_PIPELINE.md` D4.* The earlier hard Voyage-1024 commit ("bge-m3 is a false economy") was an uncited prior; Gate-1 measured no recall. The embedder is a versioned config seam: run recall@20-vs-brute-force (Voyage-1024 vs bge-m3/nomic-768) on the golden + a messy floor corpus, pick **one shared default** for the mainline cohort (a per-edition default would fracture the feedback flywheel), and keep 768-d local as a scoped opt-in for offline / the v2 privacy tier. Embedding dimension is an explicit schema/index dimension. |
...
### 3b. Model runtime — local-first dev & agentic orchestration

- **Local LLMs are a first-class option, behind `model_router`.** Because every model call is routed (not hardcoded at call sites), swapping Anthropic for local models (**Ollama** — Qwen2.5 / Llama 3.1 for extract/reason/verify; `nomic-embed-text` or `bge-m3` for embeddings) is a config change. Use local for dev, the floor experiment, and cost-free iteration. **Validate against the eval harness before trusting local models for the reasoning + verifier stages** — those are the moat and small models are weaker at non-obviousness judgment. A **hybrid** (local for extract/embed/cluster, API for reason/verify) is the expected sweet spot and is natively supported.
- **The core loop is a deterministic pipeline, not autonomous agents.** This is deliberate: precision, cost-control, and eval reproducibility all depend on fixed, inspectable stages. Free-roaming agents would degrade all three. If agent-framework ergonomics are wanted (retries, checkpointing, observability), express the pipeline on an **orchestration graph (LangGraph)** — same stages, graph runtime. **Reserve autonomous, multi-step agents for a v2 "deep connection explorer"** (multi-hop chains: A↔B, B↔C ⇒ is A↔C real?) — an additive feature, never the core matching loop.
